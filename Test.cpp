...
void draw_multiline_text(const string &text, color col, int x, int y, int line_h = 22)
{
    ...
    {
        size_t nl = text.find('\n', start);
        string line_str = (nl == string::npos)
                              ? text.substr(start)
...
string gui_input_textbox(const string &prompt, int x, int y, int w, int h)
{
    ...
    while (!finished)
    {
        ...
        bool shift = key_down(LEFT_SHIFT_KEY) || key_down(RIGHT_SHIFT_KEY);
        for (int k = 0; k < 256; k++)
        {
...
string gui_input_text(const string &prompt, int x, int y)
{
    ...
    while (!finished)
    {
        ...
        // Kiểm tra các phím từ A-Z, 0-9 và một số ký tự
        for (int k = 0; k < 256; k++)
        {
            if (key_typed((key_code)k))
            {
                if (k == RETURN_KEY) // Enter
                {
                    finished = true;
                    break;
                }
                else if (k == BACKSPACE_KEY) // Backspace
                {
                    if (!input.empty())
                        input.pop_back();
                }
...
bool is_unit_already_used(const StudentRecord &student, const string &unit_code)
{
    for (int i = 0; i < NUM_SUBJECTS; i++)
        if (student.math[i].code == unit_code)
            return true;
    for (int i = 0; i < NUM_SUBJECTS; i++)
        if (student.data[i].code == unit_code)
            return true;
    for (int i = 0; i < NUM_SUBJECTS; i++)
        if (student.programming[i].code == unit_code)
            return true;
    return false;
}
...
void gui_input_subject(StudentRecord &student, Subject &subj, const string &category, int number)
{
    const string *valid_units;
    int count;
    category_pick(category, valid_units, count); // dùng hàm của cậu

    if (valid_units == nullptr || count == 0)
    {
        gui_message_box("Error", "No valid units found for category " + category);
        return;
    }
    ...
    // Show full list once in a single dialog
    ...
    while (!valid_code)
    {
        string unit_code = gui_read_string_nonempty("Course code for " + category + " #" + to_string(number));
        unit_code = trim(unit_code);

        for (auto &c : unit_code)
...
void total_grade(const StudentRecord &student, int &math_total_grade, int &data_total_grade, int &programming_total_grade)
{
    math_total_grade = data_total_grade = programming_total_grade = 0;
    for (int i = 0; i < NUM_SUBJECTS; i++)
        math_total_grade += student.math[i].grade;
    for (int i = 0; i < NUM_SUBJECTS; i++)
        data_total_grade += student.data[i].grade;
    for (int i = 0; i < NUM_SUBJECTS; i++)
        programming_total_grade += student.programming[i].grade;
}

void gui_suggest_job(int math_total_grade, int data_total_grade, int programming_total_grade)
{
    string out;
    // Data
    if (data_total_grade <= 4)
        out += "Your data skills are just acceptable, try to improve them.\n";
    if (data_total_grade >= 5)
        out += "- Junior Data Entry Clerk\n";
    if (data_total_grade >= 7)
        out += "- Business Intelligence Assistant\n";
    if (data_total_grade >= 11)
        out += "- Statistician / Research Analyst\n";
    if (data_total_grade >= 13)
        out += "- Machine Learning Engineer\n";
    if (data_total_grade >= 15)
        out += "- Data Engineer\n";
    if (data_total_grade >= 16)
        out += "- Data Scientist\n";

    // Math
    if (math_total_grade <= 4)
        out += "Your math skills are just acceptable, try to improve them.\n";
    if (math_total_grade >= 5)
        out += "- Math Tutor / Teaching Assistant\n";
    if (math_total_grade >= 7)
        out += "- Actuarial Assistant\n";
    if (math_total_grade >= 9)
        out += "- Operations Research Analyst\n";
    if (math_total_grade >= 11)
        out += "- Statistician\n";
    if (math_total_grade >= 13)
        out += "- Quantitative Analyst\n";
    if (math_total_grade >= 15)
        out += "- Theoretical Mathematician / Researcher\n";
    if (math_total_grade >= 16)
        out += "- Data Scientist / AI Researcher\n";

    // Programming
    if (programming_total_grade <= 4)
        out += "Your programming skills are just acceptable, try to improve them.\n";
    if (programming_total_grade >= 5)
        out += "- IT Support Assistant\n";
    if (programming_total_grade >= 7)
        out += "- Web Developer\n";
    if (programming_total_grade >= 9)
        out += "- Mobile App Developer\n";
    if (programming_total_grade >= 11)
        out += "- Software Developer\n";
    if (programming_total_grade >= 13)
        out += "- Game Developer\n";
    if (programming_total_grade >= 15)
        out += "- Software Engineer\n";
    if (programming_total_grade >= 16)
        out += "- AI / Machine Learning Engineer\n";

    if (data_total_grade <= 4 && math_total_grade <= 4 && programming_total_grade <= 4)
        out += "Focus on improving your grades.\n";
    if (data_total_grade == 16 && math_total_grade == 16 && programming_total_grade == 16)
...
{
    user.username = gui_read_string_nonempty("Username:");
    user.password = gui_read_string_nonempty("Password:");
    ...
    infile.close();
    return user.password == file_password;
}

bool signup(UserAccount &user)
{
    user.username = gui_read_string_nonempty("Create username:");
    string filename = user.username + ".txt";

    ifstream infile(filename);
    if (infile)
    {
        gui_message_box("Exists", "Account already exists!");
        return false;
    }
    infile.close();

    user.password = gui_read_string_nonempty("Create password:");
    ofstream outfile(filename);
    if (!outfile)
    {
        gui_message_box("Error", "Cannot create account file.");
        return false;
    }
    outfile << user.password;
    outfile.close();
    gui_message_box("Success", "Signup successful!");
    return true;
}
...
bool signup_nonblocking(const UserAccount &user)
{
    ...
    string safe = sanitize_filename(user.username);
    string filename = "users/" + safe + ".txt";
    ...
    string hashed = hash_password_simple(user.password);
...
bool login_nonblocking(const UserAccount &user)
{
    ...
    string provided_hash = hash_password_simple(user.password);
...
int main()
{
    ...
    Button sign_in{200, 250, 100, 40, "Sign In"};
    Button sign_up{350, 250, 100, 40, "Sign Up"};
    Button back_btn{20, 300, 80, 30, "Back"};
    Button logout_btn{540, 10, 80, 30, "Logout"};
    Button create_btn{300, 250, 120, 40, "Create"};
    ...
    while (!done)
    {
        ...
        // handle mouse clicks to set focus or press buttons
        if (mouse_clicked(LEFT_BUTTON))
        {
            // global textbox areas
            ...
            else if (current == Screen::SIGN_UP)
            {
                ...
                if (mouse_x() >= create_btn.x && mouse_x() <= create_btn.x + create_btn.w && mouse_y() >= create_btn.y && mouse_y() <= create_btn.y + create_btn.h)
                {
                    UserAccount u{username, password};
                    if (signup_nonblocking(u))
                    {
                        ...
                        current = Screen::SIGN_IN;
                        password.clear(); // optional: clear password after create
                        focus = Focus::NONE;
                    }
                    else
            ...
            else if (current == Screen::ENTER_SUBJECTS)
            {
                ...
                else
                {
                    ...
                    int y = 50;
                    gui_input_text("=== MATH ===", 50, y);
                    for (int i = 0; i < NUM_SUBJECTS; i++)
                    {
                        gui_input_subject(student, student.math[i], "Math", i + 1); // gui version
                    }

                    gui_input_text("=== DATA ===", 50, y);
                    for (int i = 0; i < NUM_SUBJECTS; i++)
                    {
                        gui_input_subject(student, student.data[i], "Data", i + 1);
                    }

                    gui_input_text("=== PROGRAMMING ===", 50, y);
                    for (int i = 0; i < NUM_SUBJECTS; i++)
                    {
                        gui_input_subject(student, student.programming[i], "Programming", i + 1);
                    }

                    gui_input_text("=== Job Suggestion ===", 50, y);
                    gui_input_text("Hi " + username + ". Based on your performance. We suggest you these jobs", 50, y);

                    int math_total, data_total, programming_total;
                    total_grade(student, math_total, data_total, programming_total);
                    gui_suggest_job(math_total, data_total, programming_total);

                    gui_input_text("Hope this will help you!", 50, 50);
                }
            }
        } // end mouse_clicked

        // keyboard input for focused textbox (non-blocking)
        if (focus != Focus::NONE)
        {
            // backspace
            ...
            // Enter/Return: if on password and on SignIn screen, submit login
            if ((key_typed(RETURN_KEY) || key_typed(RETURN_KEY)) && focus == Focus::PASSWORD && current == Screen::SIGN_IN)
            {
                UserAccount u{username, password};
                if (login_nonblocking(u))
                {
        ...
        if (current == Screen::SIGN_IN)
        {
            ...
            string uname_display = username;
            if (uname_display.empty() && focus != Focus::USERNAME)
                uname_display = "(click to type)";
